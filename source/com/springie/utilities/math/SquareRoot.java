...
public final class SquareRoot {
  static final int[] table = {0, 16, 22, 27, 32, 35, 39, 42, 45, 48, 50, 53,
    55, 57, 59, 61, 64, 65, 67, 69, 71, 73, 75, 76, 78, 80, 81, 83, 84, 86, 87,
    89, 90, 91, 93, 94, 96, 97, 98, 99, 101, 102, 103, 104, 106, 107, 108, 109,
   ...
   * where it is presented by Ben Discoe (rodent@netcom.COM) Not terribly
   * speedy...
   */

  /*
   * static int unrolled_sqrt(int x) { int v; int t = 1 < <30; int r = 0; int s;
   * 
   * s = t + r; r>>= 1; if (s <= x) { x -= s; r |= t;} t >>= 2; s = t + r; r>>=
   * 1; if (s <= x) { x -= s; r |= t;} t >>= 2; s = t + r; r>>= 1; if (s <= x) {
   * x -= s; r |= t;} t >>= 2; s = t + r; r>>= 1; if (s <= x) { x -= s; r |= t;}
   * t >>= 2; s = t + r; r>>= 1; if (s <= x) { x -= s; r |= t;} t >>= 2; s = t +
   * r; r>>= 1; if (s <= x) { x -= s; r |= t;} t >>= 2; s = t + r; r>>= 1; if (s <=
   * x) { x -= s; r |= t;} t >>= 2; s = t + r; r>>= 1; if (s <= x) { x -= s; r |=
   * t;} t >>= 2; s = t + r; r>>= 1; if (s <= x) { x -= s; r |= t;} t >>= 2; s =
   * t + r; r>>= 1; if (s <= x) { x -= s; r |= t;} t >>= 2; s = t + r; r>>= 1;
   * if (s <= x) { x -= s; r |= t;} t >>= 2; s = t + r; r>>= 1; if (s <= x) { x -=
   * s; r |= t;} t >>= 2; s = t + r; r>>= 1; if (s <= x) { x -= s; r |= t;} t
   * >>= 2; s = t + r; r>>= 1; if (s <= x) { x -= s; r |= t;} t >>= 2; s = t +
   * r; r>>= 1; if (s <= x) { x -= s; r |= t;} t >>= 2; s = t + r; r>>= 1; if (s <=
   * x) { x -= s; r |= t;}
   * 
   * return r; }
  ...
  /*
   * static int mborg_sqrt(int val) { int guess=0; int bit = 1 < < 15; do {
   * guess ^= bit; // check to see if we can set this bit without going over
   * sqrt(val)... if (guess * guess > val ) guess ^= bit; // it was too much,
   * unset the bit... } while ((bit >>= 1) != 0);
   * 
   * return guess; }
   */

  /**
   * Taken from http://www.jjj.de/isqrt.cc Code not tested well... Attributed
   * to: http://www.tu-chemnitz.de/~arndt/joerg.html / email:
   * arndt@physik.tu-chemnitz.de Slow.
   */

  /*
   * final static int BITS = 32; final static int NN = 0; // range:
   * 0...BITSPERLONG/2
   * 
   * final static int test_sqrt(int x) { int i; int a = 0; // accumulator... int
   * e = 0; // trial product... int r;
   * 
   * r=0; // remainder...
   * 
   * for (i=0; i < (BITS/2) + NN; i++) { r < <= 2; r += (x >> (BITS - 2)); x < <=
   * 2;
   * 
   * a < <= 1; e = (a < < 1)+1;
   * 
   ...
   * 
   * static int test_sqrt(int n) { float r =
   * 2.0F; float s = 0.0F; for(; r < (float)n / r; r *= 2.0F); for(s = (r +
   * (float)n / r) / 2.0F; r - s > 1.0F; s = (r + (float)n / r) / 2.0F) { r = s; }
   * 
   * return (int)s; }
   */
   ...
   * // Algorithm by John.Halleck@utah.edu
   * 
   * public static int isqrt(int x) { int squaredbit; int remainder; int root;
   * 
   * if (x < 1) { return 0; }
   * 
   * squaredbit = 0x40000000; remainder = x; root = 0; while (squaredbit > 0) {
   * if (remainder >= (squaredbit | root)) { remainder -= squaredbit | root;
   * root >>= 1; root |= squaredbit; } else { root >>= 1; } squaredbit >>= 2; }
   * 
   * return root; }
   *  
   */
