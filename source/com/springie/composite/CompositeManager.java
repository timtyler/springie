...
public class CompositeManager {
  ...
  //
  // // if (temp4 < 0) debug("temp4 < 0 - egg?"); // NOT OK - this should be
  // alive still...
  // // if (temp5 < 0) debug("temp5 < 0 -- egg?"); // unless it's an attached
  // egg...
  //
  // if (temp4 >= 0) { // part of the problem...
  // if (temp5 >= 0) { // part of the problem...
  //
  // if (!pw.link_manager.isThereALinkFromTo(current_creature.node_list[temp4],
  // current_creature.node_list[temp5])) {
  // temp_link2 = pw.link_manager.setLink(current_creature.node_list[temp4],
  // current_creature.node_list[temp5], temp_link);
  // //if (FrEnd.big_babies) {
  ...
  final void killNumberedCreature(int n) {
    if ((n >= 0) && (n < this.number_of_creatures)) {
      ...
      if ((temp_creature.status & Composite.DEAD) == 0) {
        ...
        // don't try to kill a creature twice...
  ...
  // else {}
  // never fires...
  // debug("ATTEMPT TO KILL DEAD CREATURE(" + n + ") ERROR - NOC:" +
  // this.number_of_creatures);
  // debug("CREATURE NUMBER(" + n + ") KILLED - Now NOC = " +
  // this.number_of_creatures);
  // else
  // {
  // now fires...
  // debug("CREATURE NUMBER(" + n + ") OUT OF BOUNDS - NOC:" +
  // this.number_of_creatures);
  // }
   ...
   * for (temp = this.number_of_creatures; --temp >= 0; ) { temp_creature =
   * creature[temp];
   * 
   * if (temp_creature.private_world == pw_from) { temp_creature.private_world =
   * pw_to;
   *  // move *all* the entities across (sigh)... // *must* go forwards... for
   * (temp2 = 0; temp2 < temp_creature.number_of_entities; temp2++) { temp_node =
   * temp_creature.node_list[temp2]; temp4 = pw_from.getNodeNumber(temp_node);
   * temp_creature.node_list[temp2] = pw_to.agent[temp4 + ien]; if
   * (temp_creature.node_list[temp2] == null) { debug("problems already"); //
   * doesn't fire... } }
   * 
