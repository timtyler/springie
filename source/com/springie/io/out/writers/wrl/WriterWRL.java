...
public class WriterWRL {
  ...
  private void outputLink(Link link, int number) {
    ...
    for (int i = 0; i < total - 1; i++) {
      ...
      final Point3D pos_2 = link.nodes[i + 1].pos;

      final double x = toVRMLCoords(((pos_1.x + pos_2.x) >> 1) - this.middle.x);
      final double y = toVRMLCoords(((pos_1.y + pos_2.y) >> 1) - this.middle.y);
      final double z = toVRMLCoords(((pos_1.z + pos_2.z) >> 1) - this.middle.z);

      final double rx = toVRMLCoords(pos_1.z - pos_2.z);
      final double ry = toVRMLCoords(pos_1.y - pos_2.y);
      final double rz = toVRMLCoords(pos_2.x - pos_1.x);

      //if (ry == 0.0d) {
        //ry = 1.0d;
      //}

      final double amp = Math.sqrt((rx * rx) + (rz * rz));
      final double theta = (ry == 0) ? (Math.PI / 2) : Math.atan(amp / ry);
