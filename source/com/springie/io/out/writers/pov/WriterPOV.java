...
public class WriterPOV {
    ...
    for (int c = 0; c < clazz_number; c++) {
      ...
      if (number_of_faces > 0) {
        if (o < 0.99) {
          ...
          writeLine("#declare CF" + c + " = pigment { color rgb <" + emit(r)
              + "," + emit(g) + "," + emit(b) + "> }");
        }
      }
    }
  ...
  private void outputLinkSection(Link link, int n_clazz, final Point3D pos_A,
      final Point3D pos_B) {
    ...
    // amp1));
    // } else {
    theta1 = (ry == 0) ? -degree_conversion * (Math.PI / 2)
        : -degree_conversion * Math.atan(amp1 / ry);
    // }
    // if (rx > rz) {
    // theta2 = pi_over_2 - ((rx == 0) ? pi_over_2 : -cvf * Math.atan(((double)
    // rx) / rz));
    // } else {
    theta2 = (rx == 0) ? -degree_conversion * (Math.PI / 2)
        : -degree_conversion * Math.atan(((double) rz) / rx);
    // }
  ...
  private void writeLinkObject(Link link, int n_clazz, final double x,
      final double y, final double z, final double theta1, final double theta2) {
    ...
    if (link.type.compression && bulge) {
      ...
      writeLine(" union {");
      writeLine("  cylinder { <0,-" + proportion + ",0>, <0," + proportion
          + ",0>, " + emit(radius));
      writeLine("   texture { CL" + n_clazz + " } }");
      writeLine("  cone { <0,-1.0,0>, 0, <0,-" + proportion + ",0>, "
          + emit(radius));
      writeLine("   texture { CL" + n_clazz + " } }");
      writeLine("  cone { <0,1.0,0>, 0, <0," + proportion + ",0>, "
          + emit(radius));
