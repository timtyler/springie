...
public class WriterSpr {
  ...
  public String generateString() {
    ...
    uni.add(new XMLWriterAttribute("continuously_centre_x", "" + FrEnd.continuously_centre_x));
    uni.add(new XMLWriterAttribute("continuously_centre_y", "" + FrEnd.continuously_centre_y));
    uni.add(new XMLWriterAttribute("continuously_centre_z", "" + FrEnd.continuously_centre_z));
  ...
  private void outputColour(final Clazz clazz, final XMLWriterTagPair tag_type) {
    ...
    if (t < 0.999f) {
      tag_type.add(new XMLWriterAttribute("opacity", WriteFloatingPoint.emit(t)));
    }
    final float r = ((clazz.colour >>> 16) & 0xFF) / 255f;
    if (r < 0.999f) {
      tag_type.add(new XMLWriterAttribute("red", WriteFloatingPoint.emit(r)));
    }

    final float g = ((clazz.colour >>> 8) & 0xFF) / 255f;
    if (g < 0.999f) {
      tag_type.add(new XMLWriterAttribute("green", WriteFloatingPoint.emit(g)));
    }

    final float b = (clazz.colour & 0xFF) / 255f;
    if (b < 0.999f) {
      tag_type.add(new XMLWriterAttribute("blue", WriteFloatingPoint.emit(b)));
    }
  }
  ...
  private boolean outputLinkTypes(final XMLWriterTagPair uni, Clazz clazz) {
    ...
    for (int lt = 0; lt < link_type_number; lt++) {
      ...
      if (link_type.radius != 0) {
        tag_type.add(new XMLWriterAttribute("radius", "" + scale(link_type.radius)));
      }
      if (link_type.elasticity != LinkType.default_elasticity) {
        tag_type.add(new XMLWriterAttribute("elasticity", "" + link_type.elasticity));
      }
      ...
      if (!link_type.tension) {
        tag_type.add(new XMLWriterAttribute("tension", "false"));
      }
      if (!link_type.compression) {
        tag_type.add(new XMLWriterAttribute("compression", "false"));
      }
      if (link_type.hidden) {
        tag_type.add(new XMLWriterAttribute("hidden", "true"));
      }
  ...
  private boolean outputFaceTypes(final XMLWriterTagPair uni, Clazz clazz) {
    ...
    for (int t = 0; t < type_number; t++) {
      ...
      final boolean any = outputFace(face_manager, clazz, polygon_type, tag);
      if (any) {
        uni.add(tag);
        some = true;
      }
  ...
  private XMLWriterSinglet outputLink(Link link) {
    ...
    tag_link.add(new XMLWriterAttribute("nodes", "" + nodes));
  ...
  XMLWriterSinglet getNodePosition(final Node node) {
    final XMLWriterSinglet singlet = new XMLWriterSinglet("position");

    final int y = Coords.getInternalFromPixelCoords(Coords.z_pixels) - node.pos.z;
