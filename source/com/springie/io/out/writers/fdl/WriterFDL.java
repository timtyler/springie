...
public class WriterFDL {
	...
	private boolean outputLinkTypes(final FDLObjectBraceList uni, Clazz clazz) {
		...
		for (int lt = 0; lt < link_type_number; lt++) {
			...
			chain_elasticity.add(new FDLObjectNumber("" + scale(link_type.elasticity)));
			...
			final boolean any = outputLinks(link_manager, clazz, link_type, list_links);
	...
	private boolean outputFaceClasses(FDLObjectBraceList polygons) {
		...
		boolean some = false;
		for (int c = 0; c < clazz_number; c++) {
			final Clazz clazz = (Clazz) this.node_manager.clazz_factory.array.get(c); // TODO NODE?
