...
public class LineFragment {
  ...
  LineFragment(int ix, int iy, int jx, int jy, int idx, int idy, int rotspeed) {
    ...
    final int jx2 = (jx - ix) >> Coords.shift;
    final int jy2 = (jy - iy) >> Coords.shift;

    this.length = (byte) ((SquareRoot.fastSqrt(1 + (jx2 * jx2) + (jy2 * jy2)) >> 1));
  ...
  void draw() {
    ...
    RendererDelegator.graphics_handle.drawLine(temp_x - temp_dx, temp_y - temp_dy, temp_x
