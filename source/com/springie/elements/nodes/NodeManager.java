...
public class NodeManager extends World {
  ...
  public void nodeAndLinkRenderDummy() {
    ...
    for (int counter = number_of_nodes; --counter >= 0;) {
      ...
      cached_node.renderDummyLinks(this.renderer.renderer_link,
          getLinkManager(), node, mask);
      // }

      // if (FrEnd.render_polygons) {
      cached_node.renderDummyPolygons(this.renderer.renderer_face,
          getFaceManager(), node, mask);
      // }
    }
  }
  ...
  public void nodeAndLinkRenderNormal() {
    ...
    for (int counter = number_of_nodes; --counter >= 0;) {
      ...
      if (FrEnd.render_links) {
        cached_node.renderLinks(this.renderer.renderer_link, getLinkManager(),
            node, mask);
      }

      if (FrEnd.render_faces) {
        cached_node.renderPolygons(this.renderer.renderer_face,
            this.getFaceManager(), node, mask);
      }
    }
  }
  ...
  public void nodeAndLinkRenderAnaglyph() {
    ...
    for (int counter = number_of_nodes; --counter >= 0;) {
      ...
      if (FrEnd.render_links) {
        Coords.shift_constant_x -= anaglyph_distance;
        cn1.renderLinks(this.renderer.renderer_link, getLinkManager(), n,
            mask_right);
        Coords.shift_constant_x += anaglyph_distance << 1;
        cn2.renderLinks(this.renderer2.renderer_link, getLinkManager(), n,
            mask_left);
        Coords.shift_constant_x -= anaglyph_distance;
      }
      ...
      if (FrEnd.render_faces) {
        Coords.shift_constant_x -= anaglyph_distance;
        cn1.renderPolygons(this.renderer.renderer_face, getFaceManager(), n,
            mask_right);
        Coords.shift_constant_x += anaglyph_distance << 1;
        cn1.renderPolygons(this.renderer2.renderer_face, getFaceManager(), n,
            mask_left);
        Coords.shift_constant_x -= anaglyph_distance;
      }
    }
  ...
  public final void findNewBins() {
    ...
    for (int counter = number_of_nodes; --counter >= 0;) {
      ...
      if ((temp_agent.current_bin.x != new_bin_x)
        ...
        this.node_grid.removeFromList(temp_agent.current_bin.x,
