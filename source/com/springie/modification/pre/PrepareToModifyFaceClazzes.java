...
public class PrepareToModifyFaceClazzes {
  ...
  private boolean thereIsALinkClazzClash() {
    ...
    for (int c1 = number; --c1 >= 0;) {
      for (int c2 = c1; --c2 >= 0;) {
        final Face l1 = (Face) this.face_manager.element.get(c1);
        final Face l2 = (Face) this.face_manager.element.get(c2);
  ...
  public void makeNewLinkClazzes() {
    ...
    for (int counter = number; --counter >= 0;) {
      final Face candidate = (Face) this.face_manager.element.get(counter);
