...
public final class PresetObjects {
  ...
  public static String getCreatureDescription(ArgumentList arguments) {
    ...
    } else if (type == ProceduralObject.string) {
      final int length = ((Integer) arguments.getArguments(1)).intValue();
      return makeString(length, 10);
...
//    }
  ...
  static String makeString(int nodes, int number) {
    ...
    //for (int j = 0; j < number; j++) {
      dome += getSubtring(nodes, colours, scale_factor, dome);
    //}
  ...
  private static String getSubtring(int nodes, final int[] colours, final int scale_factor, String dome_in) {
    ...
    final int iz = 8000 + rnd.nextInt(1999);

    int i = 0;
    final int length = 8 * 256 * scale_factor;

    for (int x = 0; x < nodes; x++) {
      final int charge = rnd.nextInt(3);
      ...
      dome += "LG L:" + length + " R:" + (length >> 3) + " ";
      dome += "E:60 ";
      final int colour = colours[((x % 1) * 3) + 0] | 0x808080;
      dome += "C:" + colour + " ";
